// ===========================
// script.js - UBT App Update (Full Version with Native Android Bridge + Fix Mobile POST)
// ===========================
...
// ===========================
// Kirim skor ke Sheet (FIXED untuk mobile: cors + header + delay + fallback GET)
// ===========================
function sendScoreToSheet(username, paket, score) {
    ...
    // DETEKSI APAKAH DI APK ANDROID (ada window.Android)
    if (window.Android) {
    ...
    // FIX utama: pakai mode "cors" + header + logging
    ...
    .then(response => {
        console.log("Response dari sheet:", response.status, response.ok ? "OK" : "GAGAL");
        if (response.ok) {
            console.log("✅ Skor terkirim sukses via POST!");
        } else {
            console.error("POST gagal, status:", response.status);
        }
    })
    .catch(err => {
        console.error("❌ Fetch POST error:", err.message);
        // Fallback GET kalau POST tetep gagal
        const getUrl = SPREADSHEET_URL + '?' + formData.toString();
        fetch(getUrl, { method: "GET", keepalive: true, mode: "no-cors" })
            .then(() => console.log("✅ Fallback GET berhasil"))
            .catch(e => console.error("Fallback GET juga gagal:", e));
    });
}
...
// ===========================
// Finish Exam (dengan native bridge support + DAFTAR SOAL SALAH)
// ===========================
function finish() {
    ...
    // --- TAMBAHAN: Kumpulin daftar soal yang salah ---
    let wrongList = [];
    questions.forEach(q => {
        const userAnswer = answered[q.id];
        if (userAnswer && userAnswer !== q.answer) {
            wrongList.push(`Soal ${q.id}: jawaban Anda ${userAnswer} → benar ${q.answer}`);
        }
    });

    // Bersihkan localStorage tapi simpan data sent key
    const sentKeys = Object.keys(localStorage).filter(k => k.startsWith("ubt_sent_"))
        .reduce((obj,k)=>{obj[k]=localStorage.getItem(k);return obj;}, {});
    localStorage.clear();
    Object.entries(sentKeys).forEach(([k,v])=>localStorage.setItem(k,v));

    // Tampilkan daftar soal salah (jika ada)
    let wrongMessage = "Semua jawaban benar! Mantap 🔥";
    if (wrongList.length > 0) {
        wrongMessage = "Soal yang salah:\n" + wrongList.join("\n");
        if (wrongList.length > 15) {
            wrongMessage += `\n\n...dan ${wrongList.length - 15} soal lainnya.`;
        }
    }

    // Delay redirect supaya kirim sempat jalan di HP
    setTimeout(() => { 
        alert(wrongMessage);  // ← Alert daftar yang salah dulu
        
        alert(`Ujian selesai!\nNilai Anda: ${score}\nData sudah dikirim ke pusat! 🎉`);
        
        location.href = "index.html"; 
    }, 800); // 800ms lebih aman di mobile
}
